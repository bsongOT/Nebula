...
export function NebulaPalette(data: { contents: DataCollection<Content> }, info: { nebula:CommonNebula }) {
    ...
    return contents
      .map(c => ({
        element: li({
          ...
          className: U(() => `${selectedContents.includes(c) ? "selected" : ""} ${info.nebula.tree.nodes.map(n => n.data).includes(c) ? "dead" : ""}`.trim()) 
        })(),
        content: c,
  ...
  return div({class: "nebula-palette"})([
    ...
    div({className: U(() => isInputOpened ? "palette-text-box" : "palette-text-box hidden")})([
      textarea({
        class: "palette-text", 

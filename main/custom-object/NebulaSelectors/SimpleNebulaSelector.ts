...
export class SimpleNebulaSelector extends UIManager {
    ...
    constructor(attributes:SimpleNebulaSelectorInfo){
        ...
        this.info.capacity = this.info.capacity ?? 15;
        this.layout = {
    ...
    public update() {
        ...
        if (this.info.page && this.info.capacity) {
    ...
    public detect() {
        ...
        if (this.info.page && !this.layout.pageCounter.innerText.startsWith(this.info.page.toString())) return true;
        if (this.info.capacity && this.layout.capacity.value !== this.info.capacity?.toString()) return true;
